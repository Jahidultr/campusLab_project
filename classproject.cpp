...
{
	MessageBox(NULL, "Team_D64_Batch is a WIN32 application developed for analysing \"Aging\" as a solution to starvation (an issue associated with priority based scheduling algorithms).", "Welcome to PLANISTA", MB_OK);
	WNDCLASSW wc = { 0 };
	wc.hbrBackground = (HBRUSH)COLOR_WINDOW;
	wc.hCursor = LoadCursor(NULL, IDC_ARROW);
	wc.hInstance = hInst;
	wc.lpszClassName = L"Team_D64_Batch_window_class";
	wc.lpfnWndProc = WindowProcedure;
	if (!RegisterClassW(&wc))
		return -1;
	CreateWindowW(L"Team_D64_Batch_window_class", L"Team_D64_Batch win32 application", WS_OVERLAPPEDWINDOW | WS_VISIBLE, 100, 50, 600, 750, NULL, NULL, NULL, NULL);
	MSG msg = { 0 };
...
{
	//heading
	CreateWindowW(L"Static", L"Team_D64_Batch", WS_VISIBLE | WS_CHILD | SS_CENTER, 0, 15, 600, 19, hWnd, NULL, NULL, NULL);
	//AT,BT,P
	...
	//Type of scheduling
	CreateWindowW(L"button", L"Pre-emptive", WS_VISIBLE | WS_CHILD, 100, 300, 175, 40, hWnd, (HMENU)Pre_emptive, NULL, NULL);
	CreateWindowW(L"button", L"Non-Pre-emptive", WS_VISIBLE | WS_CHILD, 355, 300, 175, 40, hWnd, (HMENU)Non_Pre_emptive, NULL, NULL);
	//CPU SCHEDULE LOGGER
	CreateWindowW(L"Static", L"CPU SCHEDULE LOGGER:", WS_VISIBLE | WS_CHILD | SS_CENTER, 0, 380, 600, 15, hWnd, NULL, NULL, NULL);
	PSL = CreateWindowW(L"edit", L"", WS_VISIBLE | WS_CHILD | ES_MULTILINE | ES_AUTOVSCROLL, 100, 420, 430, 220, hWnd, NULL, NULL, NULL);
}
void PE1()
{
...
void NPE2()
{
    ...
    sprintf(temporary,"\r\n\r\tAverage Turn Around Time (TAT) is %.2f",(float)sum_TAT/n);
    strcat(buffer,temporary);
    sprintf(temporary,"\r\n\r\tAverage Waiting Time (WT) is %.2f",(float)sum_WT/n);
    strcat(buffer,temporary);
    SetWindowText(PSL,buffer);
}
void GetValues()
